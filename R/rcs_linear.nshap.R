...
rcs_linear.nshap<-function(data,knot,y,x,covs,prob,filepath,...)
{
  ...
  nshapcimax <- subset(nshapci, yhat==0 )
  ...
  xtitle <- x
  ytitle1 <- paste0("continue y where the nshap.cutoff for ", x, " is ", sprintf("%.3f", nshap.cutoff))
  ...
  plot.nshap.type1 <- ggplot2::ggplot() +
    ...
    geom_point(aes(x<<-nshap.cutoff, y=0), color="#e23e57", size=2) +
    geom_segment(
      aes(
  ...
  dev.new()
  ggsave("fig.linear_nshapall.PDF", fig.nshapall, width = 14, height =10  , device = cairo_pdf, family = "Times",path=filepath)
  dev.off()
