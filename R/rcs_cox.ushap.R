...
rcs_cox.ushap<-function(data,y,x,time,covs,prob,filepath,...)
{
  ...
  xtitle <- x
  ytitle1 <- paste0("HR where the ushap.cutoff for ", x, " is ", sprintf("%.3f", ushap.cutoff))
  ...
  dev.new()
  ggsave("fig.cox_ushapall.PDF", fig.ushapall, width = 14, height =10  , device = cairo_pdf, family = "Times",path=filepath)
  dev.off()

  message.print2 <- list(aics=aics,kn=kn,
                        phassump=phassump,phresidual=phresidual,
                        Q20=Q20,ushapcimin=ushapcimin)
