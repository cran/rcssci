...
#'# library(rcssci
#'# rcs_cox.prob(data=sbpdata, y = "status",x = "sbp",covs=c("age"),
...
rcs_cox.prob<-function(data,y,x,time,covs,prob,filepath,...)
{
  ...
  xtitle <- x
  ytitle1 <- paste0("HR where the refvalue for ", x, " is ", sprintf("%.3f", refvalue))
  ...
  dev.new()
  ggsave("fig.cox_proball.pdf", fig.proball, width = 14, height =10 , device = cairo_pdf, family = "Times",path=filepath)
  dev.off()

  message.print1 <- list(aics=aics,kn=kn,
                        phassump=phassump,phresidual=phresidual,
                        Q20=Q20)
  return(message.print1)
}
