...
rcs_quasipoisson.lshap<-function(data,knot,y,x,covs,prob,filepath,...)
{
  ...
  xtitle <- x
  ytitle1 <- paste0("RR where the lshap.cutoff for ", x, " is ", sprintf("%.3f", lshap.cutoff))
  ...
  dev.new()
  ggsave("fig.quasipoisson_lshapall.PDF", fig.lshapall, width = 14, height =10  , device = cairo_pdf, family = "Times",path=filepath)
  dev.off()

  message.print <- list(aics=aics,kn=kn,Q20=Q20,lshapcicross=lshapcicross)
