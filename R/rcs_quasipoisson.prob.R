...
rcs_quasipoisson.prob<-function(data,knot,y,x,covs,prob,filepath,...)
{
  ...
  xtitle <- x
  ytitle1 <- paste0("RR where the refvalue for ", x, " is ", sprintf("%.3f", refvalue))
  ...
  dev.new()
  ggsave("fig.quasipoisson_proball.pdf", fig.proball, width = 14, height =10 , device = cairo_pdf, family = "Times",path=filepath)
  dev.off()
