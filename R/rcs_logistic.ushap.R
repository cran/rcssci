...
rcs_logistic.ushap<-function(data,knot,y,x,covs,prob,filepath,...)
{
  ...
  xtitle <- x
  ytitle1 <- paste0("OR where the ushap.cutoff for ", x, " is ", sprintf("%.3f", ushap.cutoff))
  ...
  dev.new()
  ggsave("fig.logistic_ushapall.PDF", fig.ushapall, width = 14, height =10  , device = cairo_pdf, family = "Times",path=filepath)
  dev.off()
