...
rcs_linear.ushap<-function(data,knot,y,x,covs,prob,filepath,...)
{
  ...
  ushapcimin <- subset(ushapci, yhat==0 )
  ...
  xtitle <- x
  ytitle1 <- paste0("continue y where the ushap.cutoff for ", x, " is ", sprintf("%.3f", ushap.cutoff))
  ...
  plot.ushap.type1 <- ggplot2::ggplot() +
    ...
    geom_point(aes(x<<-ushap.cutoff, y=0), color="#e23e57", size=2) +
    geom_segment(
      aes(
  ...
  dev.new()
  ggsave("fig.linear_ushapall.PDF", fig.ushapall, width = 14, height =10  , device = cairo_pdf, family = "Times",path=filepath)
  dev.off()
