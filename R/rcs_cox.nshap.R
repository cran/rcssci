...
#'# library(rcssci
#'# rcs_cox.nshap(data=sbpdata, y = "status",x = "sbp",covs=c("age"),
...
rcs_cox.nshap<-function(data,y,x,time,covs,prob,filepath,...)
{
  ...
  xtitle <- x
  ytitle1 <- paste0("HR where the nshap.cutoff for ", x, " is ", sprintf("%.3f", nshap.cutoff))
  ...
  dev.new()
  ggsave("fig.cox_nshapall.PDF", fig.nshapall, width = 14, height =10  , device = cairo_pdf, family = "Times",path=filepath)
  dev.off()

  message.print3 <- list(aics=aics,kn=kn,
                        phassump=phassump,phresidual=phresidual,
                        Q20=Q20,nshapcimax=nshapcimax)
  return(message.print3)
}
